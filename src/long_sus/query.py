...
from .config import ANALYTIC_CATALOG_DB_PATH, ETA_FACTOR_GRID, XC_FACTOR_GRID
...
def _validate_query(query: ScenarioQuery) -> tuple[ScenarioQuery, CountrySpec]:
    ...
    if query.scheme_id not in {scheme["id"] for scheme in build_validation_scheme_catalog()}:
...
def _build_scenario(query: ScenarioQuery, country_spec: CountrySpec):
    ...
    return build_validation_scenario(
        query.scheme_id,
